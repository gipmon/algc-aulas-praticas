...
PtABPNode ABPMaxRep (PtABP proot){
	if(proot==NULL){ 
		Error=NO_ABP; 
		return NULL; 
	}
	...
	/*percorre os sucessivos filhos direitos até a um nó sem filho direito*/
	while(p->PtRight!=NULL)
		p = p->PtRight;
	/*retorna o elemento mais à direita da árvore, ou seja, o maior*/
	return p;
}
...
int sum = 0;
   ...
   if ((Fila = QueueCreate (sizeof (PtABP))) == NULL){
   	Error = NO_MEM;
   	return 0;
   }
...
unsigned int ABPEvenCount (PtABPNode proot){
	...
	PtABP No = proot; PtQueue Fila;
	unsigned int even = 0;

	   if (proot == NULL)
	   { Error = ABP_EMPTY; return 0; }

	   if ((Fila = QueueCreate (sizeof (PtABP))) == NULL)
	   { Error = NO_MEM; return 0; }
	   ...
	   while (!QueueEmpty (Fila))
	   {
	       ...
	       if (No->PtLeft != NULL) QueueEnqueue (Fila, &No->PtLeft);
	       if (No->PtRight != NULL) QueueEnqueue (Fila, &No->PtRight);
	   }
